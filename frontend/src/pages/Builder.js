...
export default function Builder({ token }) {
  ...
  const calculateCompletion = () => {
    // Only count completed steps that are actually in the current filtered steps
    const filteredStepIds = new Set(filteredSteps.map((step) => step.id));
    const relevantCompletedSteps = new Set(
      Array.from(completedSteps).filter((stepId) => filteredStepIds.has(stepId))
    );

    const totalSteps = filteredSteps.length;
    const completed = relevantCompletedSteps.size;
    const percentage =
  ...
  useEffect(() => {
    calculateCompletion();
  }, [completedSteps, resume, userSections]);

  // Get filtered steps based on user sections
  ...
  // Update resume completion with current template
  ...
  useEffect(() => {
    if (location.state?.isFromAI && location.state?.aiGeneratedData) {
      ...
      // CORRECTED: Create completed steps based on AI data
      const newCompletedSteps = new Set();

      // Check each step in the steps array
      steps.forEach((step) => {
        switch (step.id) {
          case "basics":
          ...
          case "declaration":
            ...
            break;
          default:
            // For other steps (experience, education), don't mark as completed
            // since AI might not provide them
            break;
        }
      });
  ...
  const updateVisibleExtraSections = (selectedSectionKeys) => {
    ...
    const visibleExtras = [];
    Object.keys(extraSectionMap).forEach((key) => {
      if (selectedSectionKeys.includes(key)) {
        visibleExtras.push(extraSectionMap[key]);
      }
    });
  ...
  const transformBackendToFrontend = (
    ...
    return {
      ...
      sectionsOrder: sectionsOrder || initialResumeState.sectionsOrder,
      data: {
  ...
  const deleteEducation = async (id) => {
    ...
    if (educationItem.id) {
      try {
        await deleteSectionItem("education", educationItem.id);
        toast.success("Education deleted successfully!");
      } catch (error) {
        console.error("Failed to delete education:", error);
        await loadResume(resumeId, true);
      }
    }
  };
  ...
  // Template application with immediate save
  ...
  const handleUpdateSections = async (newSections) => {
    try {
      ...
      // Filter completed steps to only include those in the new sections
      const newStepIds = steps
        .filter((step) => {
          const backendKey = STEP_TO_BACKEND_MAP[step.id];
          return newSections.includes(backendKey);
        })
        .map((step) => step.id);

      setCompletedSteps((prev) => {
        const newSet = new Set();
        Array.from(prev).forEach((stepId) => {
          if (newStepIds.includes(stepId)) {
            newSet.add(stepId);
          }
        });
        return newSet;
  ...
  const handleSave = async () => {
    try {
      ...
      filteredSteps.forEach((step) => {
        const backendSection = stepToBackendMap[step.id];
        if (backendSection && userSections.includes(backendSection)) {
          sectionsData[backendSection] =
            getSectionDataForBackend(step.id) || [];
        }
      });
      ...
      Object.keys(extraSectionMap).forEach((key) => {
        if (userSections.includes(key)) {
          sectionsData[key] = getSectionDataForBackend(key) || [];
        }
      });
